...
if (isset($_POST["user_username"], $_POST["user_pwd"])) {
    if (!empty($_POST["user_username"]) && !empty($_POST["user_pwd"])) {
        ...
        // user_username = '$user_username' AND user_pwd = '$user_pwd' AND user_role = 'ADMN' LIMIT 0,1";
        //     $result = $mysqli->query($query);
        //     $rowcount = mysqli_num_rows($result);
        $query = $mysqli->prepare("SELECT * FROM user WHERE user_username =? AND user_pwd =? AND user_role = 'ADMN' LIMIT 0,1");
