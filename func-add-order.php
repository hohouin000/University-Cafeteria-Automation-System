...
if ($store_closed == 0) {
    ...
    $query =  $mysqli->prepare("INSERT INTO odr (user_id,store_id,payment_id,odr_status) VALUES (?,?,?,?);");
    $query->bind_param('iiis', $_SESSION['user_id'], $store_id, $payment_id, $odr_status);
    // $result = $mysqli->query($query);
    $query->execute();
    ...
    //$result = $mysqli->query($query);
    $query = $mysqli->prepare("SELECT c.*, m.* FROM cart c INNER JOIN mitem m ON c.mitem_id = m.mitem_id WHERE c.user_id =? AND c.store_id =? ;");
    ...
    if ($odr_detail_result) {
        ...
        // $result = $mysqli->query($query);
        $query = $mysqli->prepare("DELETE FROM cart WHERE user_id =? AND store_id =?;");
        $query->bind_param('ii', $_SESSION['user_id'], $store_id);
        $query->execute();
        if (isset($_GET["response"])) {
            if ($_GET["response"] == 1) {
