...
if (isset($_POST['mitem-id'], $_POST['store-id'], $_POST['amount'], $_POST['remark'])) {
    if (!empty($_POST['mitem-id']) && !empty($_POST['store-id']) &&  !empty($_POST["amount"])) {
        ...
        if ($rowcount == 0) {
            ...
            $query =  $mysqli->prepare("INSERT INTO cart (user_id, store_id, mitem_id, cart_amount, cart_remark) VALUES (?,?,?,?,?)");
            $query->bind_param('iiiis', $user_id, $store_id, $mitem_id, $amount, $remark);
            ...
            if ($cart_store == $store_id) {
                ...
                if ($cartrowcount == 0) {
                    ...
                    // $added_result = $mysqli->query($query);
                    $query = $mysqli->prepare("UPDATE cart SET cart_amount =? WHERE user_id =? AND mitem_id =? AND store_id =?;");
                    $query->bind_param('iiii', $new_cart_amount, $user_id, $mitem_id, $store_id);
                    $added_result = $query->execute();
                }
            } else {
        ...
        if ($added_result) {
            ...
            $_SESSION["server_status"] = 0;
            header("location:store-menu.php?store_id={$store_id}&result={$added_result}");
            exit(1);
        }
    } else {
        $_SESSION["server_status"] = 0;
