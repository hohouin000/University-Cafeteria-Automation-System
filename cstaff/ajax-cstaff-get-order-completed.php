...
if (isset($_SESSION["store_id"])) {
    ...
    $query = $mysqli->prepare("SELECT * FROM odr WHERE store_id =? AND odr_status = 'CMPLT';");
    ...
    if ($rowcount > 0) {
        while ($row = $result->fetch_array()) {
            ...
            $odr_compltime = (new Datetime($row["odr_compltime"]))->format("F j, Y H:i");
